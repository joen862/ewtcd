...
class Wallet extends Model
{
    ...
    public function seed() {
        ...
        // loop through wallets, fetch balance and insert in DB
        foreach($this->wallets as $slug => $data) {
            $balance = $this->fetchBalance($data['address']);
            $notes = '';
    ...
    public function fetchBalance($address) {
        $balance = json_decode($this->explorer(['module'=>'account','action'=>'balance','address'=>$address]));
        $balance = $balance->result/1000000000000000000;
        return $balance;
    }

    public function explorer($query) {
